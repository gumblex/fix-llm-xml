...
- 🛡️ **安全保留文本内容** – 对用户指定的纯文本标签（及 CDATA 内容），自动转义并保留原始文本内容，防止自动修复时丢失。
...
内部会先调用 `find_xml_document`，使用 `xmltodict` 解析；直接解析失败时调用 `fix_xml_with_text_tags` + lxml 修复，再次解析。
...
3. 你事先知道哪些标签是“纯文本标签”（例如 `<file>`, `<answer>`），或指示 LLM 将文本内容用 CDATA 包裹。这些标签的内容不应该被当作 XML 子元素解析，而是完整的纯文本/代码片段。
